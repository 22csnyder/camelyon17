...
def rescale_logit(logit, alpha):#tested. works
    ''' returns output such that
        alpha * tf.sigmoid(logit) = tf.sigmoid(output)
    '''
    al_inv=tf.divide(1,alpha)

    A=(al_inv-1)*tf.minimum(tf.exp(logit),1)
    B=al_inv*tf.minimum(tf.exp(-logit),1)
    return -tf.log( A + B ) + tf.minimum(logit,0)
